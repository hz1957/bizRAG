...
from bizrag.service.metadata_store import MetadataStore
from bizrag.servers.biz_corpus.biz_corpus import build_excel_corpus as build_excel_corpus_tool
...
from bizrag.servers.retriever.index_backends.milvus_backend import MilvusIndexBackend
...
class KBAdmin:
    ...
    async def _sync_document_index(
        ...
        *,
        ...
        try:
            if replace_existing:
                self._delete_document_from_index(kb=kb, doc_key=str(document["doc_key"]))
    ...
    async def _sync_deleted_document_index(
        ...
        *,
        ...
        try:
            self._delete_document_from_index(kb=kb, doc_key=str(deleted_doc["doc_key"]))
            return "incremental"
        except Exception as exc:
            retriever_app.logger.warning(
    ...
    def _delete_document_from_index(self, *, kb: Dict[str, Any], doc_key: str) -> int:
        ...
        try:
            ...
            return milvus_backend.delete_by_filter(
                collection_name=kb["collection_name"],
