...
class MilvusIndexBackend(BaseIndexBackend):
    ...
    def delete_by_filter(
        ...
        *,
        ...
        target_collection = collection_name or self.collection_name
        if not target_collection:
            raise ValueError("[milvus] collection_name is required to delete rows")
        if not self._validate_collection_name(target_collection):
            raise ValueError(
                f"[milvus] Invalid collection name: '{target_collection}'. "
                "Collection names must contain only alphanumeric characters, underscores, and hyphens."
        ...
        if not filter_expr or not isinstance(filter_expr, str):
            raise ValueError("[milvus] filter_expr is required to delete rows")

        client = self._client_connect()
        if not client.has_collection(target_collection):
            self.logger.info(
                "[milvus] Collection '%s' does not exist, skip delete.",
                target_collection,
            )
            return 0

        try:
            res = client.delete(collection_name=target_collection, filter=filter_expr)
        except Exception as exc:
            raise RuntimeError(
                f"[milvus] Delete failed on '{target_collection}': {exc}"
            ) from exc

        deleted_count = 0
        if isinstance(res, dict):
            deleted_count = int(
                res.get("delete_count")
                or res.get("deleted_count")
                or res.get("count")
                or 0
            )
        self.logger.info(
            "[milvus] Deleted %s rows from '%s' using filter: %s",
            deleted_count,
            target_collection,
            filter_expr,
        )
