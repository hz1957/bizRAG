...
class Retriever:
    ...
    async def retriever_init(
        ...
        should_load_corpus_to_memory = (self.backend == "bm25") or (
            self.index_backend_name in {"faiss", "milvus"}
        )
            ...
            with open(corpus_path, "rb") as f:
                with tqdm(
                    ...
                    for i, line in enumerate(f):
                        ...
                        self.corpus_records.append(item)
        ...
        if getattr(self, "is_demo", False):
            ...
            if self.index_backend_name == "milvus":
                if not self.corpus_records:
                    err_msg = (
                        "Milvus indexing requires corpus records loaded in memory. "
                        "Please provide a valid corpus_path during retriever_init."
                    )
                    app.logger.error(err_msg)
                    raise RuntimeError(err_msg)

                if len(self.corpus_records) != embedding.shape[0]:
                    err_msg = (
                        "Embedding count does not match corpus record count: "
                        f"{embedding.shape[0]} vs {len(self.corpus_records)}"
                    )
                    app.logger.error(err_msg)
                    raise RuntimeError(err_msg)
                ...
                metadatas = []
                vec_ids = []
                for idx, item in enumerate(self.corpus_records):
                    contents.append(str(item.get("contents", "")))
                    metadatas.append(
                        {
                            k: v
                            for k, v in item.items()
                            if k not in {"contents", "embedding", "vector"}
                        }
                    )
                    vec_ids.append(str(item.get("id", idx)))
                vec_ids = np.array(vec_ids)
            else:
            ...
            try:
                ...
                if self.index_backend_name == "milvus":
                    ...
                    build_kwargs["contents"] = contents
                    build_kwargs["metadatas"] = metadatas
                self.index_backend.build_index(**build_kwargs)
    ...
    async def retriever_search_structured(
        self,
        query_list: List[str],
        top_k: int = 5,
        query_instruction: str = "",
        ...
        rets = self.index_backend.search_payload(
            query_embedding,
            top_k,
