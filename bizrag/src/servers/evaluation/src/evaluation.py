...
def rouge1_score(gt: List[str], pred: str) -> float:
    ...
    scores = []
    for g in gt_norm_ls:
        scores.append(_rouge_scorer.score(g, pred_norm)["rouge1"].fmeasure)
    return max(scores) if scores else 0.0
...
def rouge2_score(gt: List[str], pred: str) -> float:
    ...
    scores = []
    for g in gt_norm_ls:
        scores.append(_rouge_scorer.score(g, pred_norm)["rouge2"].fmeasure)
    return max(scores) if scores else 0.0
...
def rougel_score(gt: List[str], pred: str) -> float:
    ...
    scores = []
    for g in gt_norm_ls:
        scores.append(_rouge_scorer.score(g, pred_norm)["rougeL"].fmeasure)
    return max(scores) if scores else 0.0
...
def compute_metrics(
    ...
    for gt, pred in zip(gt_list, pred_list):
        for metric in metrics:
            if metric in metrics_registry:
                results[metric].append(metrics_registry[metric](gt, pred))
            else:
                app.logger.warning(
    ...
    for metric, scores in results.items():
        avg_results[f"avg_{metric}"] = sum(scores) / len(scores) if scores else 0.0
...
def _normalize_citation_map(entry: Any) -> Dict[str, List[str]]:
    if entry is None:
        return {}
    ...
    if isinstance(entry, list):
        return {"__all__": [str(item) for item in entry]}
    return {"__all__": [str(entry)]}
...
def _structured_exact_match(gt_value: Any, pred_value: Any) -> float:
    gt_num = _try_parse_float(gt_value)
    pred_num = _try_parse_float(pred_value)
    if gt_num is not None and pred_num is not None:
        return 1.0 if abs(gt_num - pred_num) < 1e-9 else 0.0
    return exact_match_score(
        [_stringify_structured_value(gt_value)],
        _stringify_structured_value(pred_value),
    )


def compute_structured_metrics(
    gt_struct_ls: List[Dict[str, Any]],
    pred_struct_ls: List[Dict[str, Any]],
    pred_citations_ls: Optional[List[Dict[str, Any]]] = None,
    gt_citations_ls: Optional[List[Dict[str, Any]]] = None,
) -> Dict[str, Any]:
    if len(gt_struct_ls) != len(pred_struct_ls):
        raise ValueError("gt_struct_ls and pred_struct_ls must have the same length")

    pred_citations_ls = pred_citations_ls or [{} for _ in pred_struct_ls]
    gt_citations_ls = gt_citations_ls or [{} for _ in gt_struct_ls]

    field_em_scores: List[float] = []
    field_f1_scores: List[float] = []
    field_acc_scores: List[float] = []
    record_em_scores: List[float] = []
    schema_valid_scores: List[float] = []
    citation_hit_scores: List[float] = []
    per_field: Dict[str, Dict[str, List[float]]] = {}

    for gt_item, pred_item, pred_citations, gt_citations in zip(
        gt_struct_ls,
        pred_struct_ls,
        pred_citations_ls,
        gt_citations_ls,
        ...
        if not isinstance(gt_item, dict):
            raise ValueError("Each gt_struct item must be a dictionary")
        if not isinstance(pred_item, dict):
            pred_item = {}

        schema_valid_scores.append(1.0 if isinstance(pred_item, dict) else 0.0)

        pred_citation_map = _normalize_citation_map(pred_citations)
        gt_citation_map = _normalize_citation_map(gt_citations)

        field_exact_matches: List[float] = []
        citation_matches: List[float] = []

        for field_name, gt_value in gt_item.items():
            pred_value = pred_item.get(field_name)
            gt_text = _stringify_structured_value(gt_value)
            pred_text = _stringify_structured_value(pred_value)

            em = _structured_exact_match(gt_value, pred_value)
            f1 = f1_score([gt_text], pred_text)
            acc = accuracy_score([gt_text], pred_text)

            field_em_scores.append(em)
            field_f1_scores.append(f1)
            field_acc_scores.append(acc)
            field_exact_matches.append(em)

            if field_name not in per_field:
                per_field[field_name] = {"em": [], "f1": [], "acc": []}
            per_field[field_name]["em"].append(em)
            per_field[field_name]["f1"].append(f1)
            per_field[field_name]["acc"].append(acc)

            gold_doc_ids = set(gt_citation_map.get(field_name) or gt_citation_map.get("__all__", []))
            pred_doc_ids = set(pred_citation_map.get(field_name) or pred_citation_map.get("__all__", []))
            if gold_doc_ids:
                citation_match = 1.0 if gold_doc_ids & pred_doc_ids else 0.0
                citation_matches.append(citation_match)

        record_em_scores.append(1.0 if field_exact_matches and all(score == 1.0 for score in field_exact_matches) else 0.0)
        if citation_matches:
            citation_hit_scores.append(sum(citation_matches) / len(citation_matches))

    results: Dict[str, Any] = {
        "avg_field_em": sum(field_em_scores) / len(field_em_scores) if field_em_scores else 0.0,
        "avg_field_f1": sum(field_f1_scores) / len(field_f1_scores) if field_f1_scores else 0.0,
        "avg_field_acc": sum(field_acc_scores) / len(field_acc_scores) if field_acc_scores else 0.0,
        "avg_record_em": sum(record_em_scores) / len(record_em_scores) if record_em_scores else 0.0,
        "schema_valid_rate": sum(schema_valid_scores) / len(schema_valid_scores) if schema_valid_scores else 0.0,
        "citation_hit_rate": sum(citation_hit_scores) / len(citation_hit_scores) if citation_hit_scores else 0.0,
    }
    for field_name, metric_values in per_field.items():
        results[f"field.{field_name}.em"] = sum(metric_values["em"]) / len(metric_values["em"])
        results[f"field.{field_name}.f1"] = sum(metric_values["f1"]) / len(metric_values["f1"])
        results[f"field.{field_name}.acc"] = sum(metric_values["acc"]) / len(metric_values["acc"])
    return results
...
def eval_with_pytrec(
    ...
    try:
        ...
        err_msg = (
            ...
            "Please install it with `pip install pytrec_eval`"
        )
...
@app.tool(
    output="pred_struct_ls,gt_struct_ls,pred_citations_ls,gt_citations_ls,save_path->eval_res"
)
def evaluate_structured(
    pred_struct_ls: List[Dict[str, Any]],
    gt_struct_ls: List[Dict[str, Any]],
    pred_citations_ls: Optional[List[Dict[str, Any]]],
    gt_citations_ls: Optional[List[Dict[str, Any]]],
    save_path: str,
) -> Dict[str, Any]:
    results = compute_structured_metrics(
...
def evaluate_trec(
    ...
    for m in metrics:
        if m not in support_metrics:
            app.logger.warning(
                f"Metric '{m}' is not recognized. "
                f"Available metrics: {', '.join(support_metrics)}."
            )
...
def evaluate_trec_pvalue(
    ...
    if ks is None:
        ks = [1, 5, 10, 20, 50, 100]
    metrics = metrics or ["mrr", "map", "recall", "ndcg", "precision"]
