...
def _convert_to_docx_with_textutil(fp: str, out_dir: str) -> Optional[str]:
    ...
    try:
        ...
        app.logger.warning(f"textutil convert failed: {fp} | {e}")
        return None
...
def _read_with_text_extractor(fp: str, command_name: str) -> Optional[str]:
    extractor_cmd = shutil.which(command_name)
    if not extractor_cmd:
        return None

    cmd = [extractor_cmd, fp]
    try:
        proc = subprocess.run(
            cmd,
            stdout=subprocess.PIPE,
        ...
        app.logger.warning(f"{command_name} read failed: {fp} | {e}")
        return None
...
async def build_text_corpus(
    ...
    def process_one_file(fp: str) -> None:
        ...
        if ext in TEXT_EXTS:
            ...
            if content is None:
                err_msg = (
                    "Legacy Word format requires one of LibreOffice/soffice, antiword, "
                    "catdoc, or macOS textutil. "
                    f"Please install one of them or convert to .docx first: {fp}. "
            ...
            rows.append(
                {
                    ...
                    "source_uri": str(fp_path.resolve()),
            ...
            text_rows.append(
                {
                    ...
                    "source_type": "pdf",
                    "source_uri": os.path.abspath(md_path),
                    "contents": md_text,
                }
            )
    ...
    for doc in tqdm(documents, desc=f"Chunking ({chunk_backend})", unit="doc"):
        ...
        for chunk in chunks:
            if use_title:
                ...
                contents = chunk.text
            meta_chunk = {
                k: v for k, v in doc.items() if k not in {"id", "contents"}
            }
            meta_chunk.update(
                {
                    "id": current_chunk_id,
