...
from bizrag.service.rustfs_event_service import replay_stored_rustfs_event
...
async def rebuild_kb(req: RebuildKBRequest) -> Dict[str, Any]:
    require_admin()
    try:
        return await run_admin_async("rebuild_kb", kb_id=req.kb_id)
    except ServiceError as exc:
        raise HTTPException(status_code=exc.status_code, detail=exc.detail) from exc
    except RuntimeError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
async def retry_task(task_id: str) -> Dict[str, Any]:
    require_admin()
    try:
        return await run_admin_async("retry_task", task_id=task_id)
    except ServiceError as exc:
        raise HTTPException(status_code=exc.status_code, detail=exc.detail) from exc
    except RuntimeError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
